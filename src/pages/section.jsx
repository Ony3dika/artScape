...
function Section() {
  ...
  const [visible, setVisible] = useState(Array(arts.length).fill(false));
      ...
      <div className='md:grid md:grid-cols-2 lg:grid-cols-4 h-fit w-full mb-5 lg:gap-1 lg:mb-10'>
        {arts.map((art, index) => (
          <motion.div
            ...
            key={index}
            className='h-96 my-5 chaos cursor-pointer relative rounded-sm border-[1px] lg:border-[0.5px] border-[#e5deda] lg:my-0 lg:h-[30rem] flex flex-col justify-around basis-full lg:basis-1/4'
          >
            <div className='ml-5'>
